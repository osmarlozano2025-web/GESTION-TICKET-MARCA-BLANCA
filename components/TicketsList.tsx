...
export const TicketsList: React.FC = () => {
  ...
  const groupFilteredTickets = useMemo(() => {
    if (selectedGroup === 'Global') return tickets;
    return tickets.filter(t => (t as any).group === selectedGroup);
              ...
              <tbody className="divide-y divide-white/5">
                {finalTickets.map((tk) => (
                  <tr key={tk.id} className="hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td className="px-6 py-5">
                      <div className={`inline - flex items - center gap - 2 px - 3 py - 1 rounded - full border ${tk.status === 'Pendiente' ? 'bg-primary/10 text-primary border-primary/20' :
